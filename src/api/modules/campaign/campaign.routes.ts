...
import { createCampaignSchema, listCampaignsQuerySchema, campaignIdParamsSchema, updateCampaignSchema, metricsQuerySchema, monthlyMetricsQuerySchema } from './campaign.validation';

const router = Router({ mergeParams: true });

// Apenas ROOT e ADMIN (ownership validado no service)
// Metrics endpoints (antes das rotas com :campaignId para evitar conflito)
router.get('/:projectId/campaigns/metrics', authMiddleware, validateRole(['ROOT', 'ADMIN']), validateRequest(metricsQuerySchema), campaignController.getMetrics);
router.get('/:projectId/campaigns/metrics/monthly', authMiddleware, validateRole(['ROOT', 'ADMIN']), validateRequest(monthlyMetricsQuerySchema), campaignController.getMonthlyMetrics);
